...
namespace ATLASS_FITNESS
{
    public partial class Frm_scanner : Form
    {
        ...
        private void btndemarer_Click(object sender, EventArgs e)
        {
            StartCamera();
            btndemarer.Enabled = false;
        ...
        private void lblarreter_Click(object sender, EventArgs e)
        {
            StopCamera();
            btndemarer.Enabled = true;
        ...
        private void ProcessQRCode(string qrCode)
        {
            ...
            {
                isScanning = false;
                int selectedSessionID = ClsSessions.GetCurrentSessionID();
                if (selectedSessionID == -1)
                {
                    MessageBox.Show("Aucune session en cours. Impossible de faire le check-in.", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                string message;
                bool success = ClsAttendance.RecordPresenceByQRCode(qrCode, selectedSessionID, out message);

                if (success)
                {
                    System.Media.SystemSounds.Asterisk.Play();
                    MessageBox.Show(message, "Entrée Enregistrée", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            try
            {
                using (OpenFileDialog openFileDialog = new OpenFileDialog())
                {
                    ...
                    if (openFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        guna2PictureBox1.Image = (Bitmap)bitmap.Clone();
